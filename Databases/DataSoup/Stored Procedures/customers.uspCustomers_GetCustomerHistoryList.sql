...
-- Procedure Name  : [uspCustomers_GetCustomerHistoryList]
...
-- Code Example    : 
/*
Exec CUSTOMERS.dbo.uspCustomers_GetCustomerHistoryList @IPI_PageNumber=1,@IPI_RowsPerPage=23,@IPVC_ModifiedIDSeq='',@IPVC_ModifiedDate='',
@IPVC_CompanyID='C0901006950'
*/
-- Revision History:
...
--============================================================================================================================
CREATE PROCEDURE [customers].[uspCustomers_GetCustomerHistoryList] (@IPI_PageNumber      int,  
                                                              @IPI_RowsPerPage     int,  
...
BEGIN --> Main Begin
  ...
  -----------------------------------------------------------------------------------
  ;WITH tablefinal AS
         (select CH.IDSeq                                               as IDSeq,
                 CH.CompanyIDSeq                                        as [Company ID],
                 CH.SiteMasterID                                        as [SiteMaster ID],
                 CH.[Name]                                              as [Name],
                 (case when (CH.StatusTypecode = 'ACTIV')
                         then 'Active'
                       else 'InActive'
                  end)                                                  as [Status],
                 CH.OrderSynchStartMonth                                as [Synch Start Month],
				 (case when (CH.CustomBundlesProductBreakDownTypeCode = 'YEBR')
							  then 'Yes'
							  else 'No'
						 end)			as [Custom Bundle Display],
                 CH.SeparateInvoiceByFamilyFlag                         as [Separate Invoice],
                 row_number() OVER(ORDER BY CH.IDSeq desc)              as RowNumber,  --> This will show latest audit changes first as users will be interested in seeing recent changes to older changes.
  ...
  select tablefinal.IDSeq,
         tablefinal.[Company ID],
         tablefinal.[SiteMaster ID],
         tablefinal.[Name],
         tablefinal.[Status],
         tablefinal.[Synch Start Month],
         tablefinal.[Custom Bundle Display],
