...
SET ANSI_NULLS ON 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt].[Lending_AuditPaymentMethodMismatch]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt].[Lending_AuditPaymentMethodMismatch]
GO
setuser N'rpt'
GO
CREATE procedure rpt.Lending_AuditPaymentMethodMismatch
	@AuditDate	datetime
