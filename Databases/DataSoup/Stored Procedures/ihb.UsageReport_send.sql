...
SET ANSI_NULLS ON 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[ihb].[UsageReport_send]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [ihb].[UsageReport_send]
GO
setuser N'ihb'
GO
CREATE procedure ihb.UsageReport_send
	@ProcessId	smallint
as
/*
————————————————————————————————————————————————————————————————————————————————
...
begin
	...
	*/
	set	@subject	= 'Corillian Usage Statistics'
	...
	**	retrieve the recipients and message and then update the message values
	*/
	set	@subject	= 'IHB Board Report Statistics';
