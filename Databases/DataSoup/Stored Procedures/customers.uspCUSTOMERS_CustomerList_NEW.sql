...
------------------------------------------------------------------------------------------------------
CREATE procedure [customers].[uspCUSTOMERS_CustomerList_NEW] (@IPI_PageNumber    int, 
                                                    @IPI_RowsPerPage   int, 
													...
													@IPB_MultiFamilyFlag bit,
                                                    @IPB_OwnerFlag bit,
													@IPB_GSAEntityFlag bit,
													@IPB_OpsSupplierFlag bit
...
BEGIN-->Main Begin
  ...
  select @LN_CHECKSUM = checksum(coalesce(@IPVC_CustomerName,''),
								 ...
								 coalesce(@IPB_MultiFamilyFlag, ''),
                                 coalesce(@IPB_OwnerFlag, ''),
								 coalesce(@IPB_GSAEntityFlag, ''),
								 coalesce(@IPB_OpsSupplierFlag, '')								 
                                );
  ---------------------------------------
  DECLARE
	@LVI_PageNumber    int, 
    ...
    @LB_MultiFamilyFlag bit,
    @LB_OwnerFlag bit,
	@LB_GSAEntityFlag bit,
	@LB_OpsSupplierFlag bit
  ...
  SELECT
    ...
    @LB_MultiFamilyFlag = @IPB_MultiFamilyFlag,
    @LB_OwnerFlag       = @IPB_OwnerFlag,
	@LB_GSAEntityFlag   = @IPB_GSAEntityFlag,
	@LB_OpsSupplierFlag = @IPB_OpsSupplierFlag
       ...
       (select  row_number() over(order by source.[Name]   asc)
        ...
        (
         select distinct
		  ...
		  AND ((@LB_MultiFamilyFlag=1 and c.MultiFamilyFlag = @LB_MultiFamilyFlag) or @LB_MultiFamilyFlag=0 )
          AND ((@LB_OwnerFlag      =1 and c.OwnerFlag       = @LB_OwnerFlag      ) or @LB_OwnerFlag      =0 )
		  AND ((@LB_GSAEntityFlag  =1 and c.GSAEntityFlag   = @LB_GSAEntityFlag  ) or @LB_GSAEntityFlag  =0 )
          AND ((@LB_OpsSupplierFlag=1 and c.OpsSupplierFlag = @LB_OpsSupplierFlag) or @LB_OpsSupplierFlag=0 )
