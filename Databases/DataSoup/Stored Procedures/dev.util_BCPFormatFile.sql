...
SET ANSI_NULLS ON 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[dev].[util_BCPFormatFile]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dev].[util_BCPFormatFile]
GO
setuser N'dev'
GO
CREATE procedure dev.util_BCPFormatFile
	@header		nvarchar(max)	
...
select	@fieldNum = count(1)
from	@format		f
full outer join
	(	select	name
		from	sys.columns
		where	object_id = isnull(object_id(@table), 0)
...
from(	select	0			as type
		...
		select	2			as Type
			,	isnull(f.rowId, 0)
			,	cast(isnull(f.rowId, c.column_id) as varchar(6))
			,	@SQLCHAR	as FileType
			,	'0'			as PrefixLength	
			,	cast(	case isnull(f.colName, '')
						when '' then 0
						else coalesce(	case isnull(c.name, '')
										when '' then null
...
order by f.type, cast(f.FileOrder as float);
