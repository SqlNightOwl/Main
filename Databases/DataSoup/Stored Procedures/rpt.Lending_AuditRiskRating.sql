...
SET ANSI_NULLS ON 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt].[Lending_AuditRiskRating]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt].[Lending_AuditRiskRating]
GO
setuser N'rpt'
GO
CREATE procedure rpt.Lending_AuditRiskRating
	@AuditDate	datetime
...
Created  :	12/01/2009
Purpose  :	Retrieves 'Audit Risk Rating' Data for SQL Reporting purposes.
History  :
...
select	OriginatingPerson
	...
	,	RiskRatingCd
	,	CurrMiAcctTypCd
	,	round(NoteIntRate * 100, 5)	as NoteIntRate
