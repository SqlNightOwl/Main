...
SET ANSI_NULLS ON 
GO
if exists (select * from dbo.sysobjects where id = object_id(N'[rpt].[onyx_TIGReferrals_summaryByUser]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [rpt].[onyx_TIGReferrals_summaryByUser]
GO
setuser N'rpt'
GO
CREATE procedure rpt.onyx_TIGReferrals_summaryByUser
	@begin_on		datetime		= null
,	@end_on			datetime		= null
,	@department_did	int				= null
,	@user_id		nvarchar(85)	= 'all'
as
/*
————————————————————————————————————————————————————————————————————————————————
...
Purpose  :	Retruns TIG Referral statistics for the subject date range and/or 
			...
			* If the user is null then all users are returned.
History  :
...
set	@user_id		= nullif(nullif(@user_id, 'all'), '');
...
from(	select	u.[user_name]
		...
		join	Onyx6_0.dbo.users		u	with (nolock)
				on	i.insert_by = u.[user_id]
		where(	u.[user_id]				= @user_id or @user_id is null	)
		and		i.incident_product_code	= @product_code
...
select	case left(d.parameter_desc, 1)
		when 'B' then 'Branchs'
		else 'Departmenst' end				as department_type
