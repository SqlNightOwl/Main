...
-- Procedure Name  : uspORDERS_Rep_GetBillingDetailsByCustomerNEW      
...
--            
-- Code Example    : Exec [dbo].[uspORDERS_Rep_GetBillingDetailsByCustomerNEW] '','','','','07/20/2008','09/20/2008'
--       
-- Revision History:      
-- Author          : Shashi Bhushan      
-- 08/08/2007      : Stored Procedure Created.      
------------------------------------------------------------------------------------------------------      
CREATE PROCEDURE [reports].[uspORDERS_Rep_GetBillingDetailsByCustomerNEW]      
                                                                  (      
...
BEGIN         
  ...
  set identity_insert #LT_BillingDetails off;   
   ...
   -------------------------------------------------------------------------- 
   select 
          (case when S.RecordIdentifier not in ('CPR','TPR') then companyname  else '' end)   as companyname,
          (case when S.RecordIdentifier not in ('CPR','TPR') then propertyname else '' end)   as propertyname,
          (case when S.RecordIdentifier not in ('CPR','TPR') then orderidseq   else '' end)   as orderidseq,
