...
-- Procedure Name  : [uspINVOICES_ReCreateMonthlyInvoiceItem]
...
--                   @LBI_OrderItemID   as  BIGINT
-- Code Example    : 
--                  Exec Invoices.DBO.[uspINVOICES_ReCreateMonthlyInvoiceItem]
-- 	                @IPVC_AccountID  = 'A0000000010',  
-- 	                @IPVC_CompanyID  = 'C0000003017',  
-- 	                @IPVC_PropertyID = 'P0000000026',
--  	                @LBI_OrderID     = 70865,
--                      @LBI_OrderItemID = 196083   

-- 11/07/2011      : TFS 1514 : Transaction is moved to SP code from UI
----------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [invoices].[uspINVOICES_ReCreateMonthlyInvoiceItem] 
                                                   (
...
BEGIN --: Main Procedure BEGIN
  ...
  DECLARE @LI_AccessOrderLineItemCount     INT 
  ...
  -------------------------------------------------------------------------------------------------  
  Create table #TEMPRecreateMonthlyInvoiceHoldingTable (IDSeq          int not null identity(1,1),
  ...
  -- IF 0 order items were found, then quit this procedure (Nothing to process).
  ------------------------------------------------------------------------------------------------- 
  SELECT  @LI_AccessOrderLineItemCount = Count(OI.IDSeq)
  ...
  IF (@SQLErrorCode <> 0) -- sql Error occured. 
    BEGIN 
      ...
      Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError @IPVC_CodeSectiON = 'SQL Error occured, @LI_AccessOrderLineItemCount variable could not be populated' 
      Return
    END  
  -------------------------------------------------------------------------------------------------
  -- IF 0 order items were found, THEN quit the procedure.
  -------------------------------------------------------------------------------------------------
  --IF @LI_AccessOrderLineItemCount = 0 
  --  BEGIN 
  --     Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError @IPVC_CodeSectiON = '@LI_AccessOrderLineItemCount is 0, No line Items to Process.'
  ...
  --------------------------------------------------------------------------------------------------------------
  SELECT DISTINCT @LVBI_OrderItemIDSeq             = OI.IDSeq,
  ...
  INNER JOIN      
   ...
   -----------------------------------
    ...
    -----------------------------------    
  GROUP BY OI.IDSeq,OI.OrderGroupIDSeq,OI.BillToAddressTypeCode,OI.BillToDeliveryOptionCode,C.SeparateInvoiceGroupNumber,CONVERT(INT,C.MarkAsPrintedFlag),OG.IDSeq,FAM.EpicorPostingCode,FAM.TaxwareCompanyCode,PRD.PrePaidFlag
  ...
  -------------------------------------------------------------------------------------------------
   IF @LI_AccessOrderLineItemCount <> 0 
      ...
      --ACCESS - Validation Query to check for following - 
          ...
          -- This is precautionary (dates should NOT be missing since @LI_AccessOrderLineItemCount is already checked WHEN populating) 
      ---------------------------------------------------------------------------------------------------------------------------
      ...
      IF Exists (SELECT ActivationEndDate 
        ...
        BEGIN 
          ...
          RETURN -- quit procedure! 
        END 
      --To Verify Contract ActivationStartDate AND ActivationEndDate are either same or start date is less than the END date.
...
--                                       ELSE Invoices.DBO.fn_SetLastDayOfMonth(ActivationEndDate) END
      ...
      WHERE  OI.ChargeTypeCode  = 'ACS' AND OI.FrequencyCode = 'MN'
         AND  (
                  (  --to handle first time activations
                        OI.NewActivationStartDate is not null           --has an annual start date
                    and (Case when OI.proratefirstmonthFlag = 1 
                                then OI.StartDate  
                              else OI.NewActivationStartDate
                         end) <= OI.TargetDate
                  )
               ...
               AND 
                 (
                      (
                       OI.LastBillingPeriodToDate is null  or
                        (
                              OI.LastBillingPeriodToDate < OI.NewActivationEndDate --has already been activated before.
                          and OI.LastBillingPeriodToDate < OI.TargetDate
                        )
                      )                
                 )
             )
		     ...
		     and    I.accountIDSeq               = @IPVC_AccountID
	          ...
	          ----------------------------------------
	                 ...
	                 FROM   Invoices.DBO.IDGenerator WITH (NOLOCK)
                         WHERE  TypeIndicator  = 'I'
                      COMMIT TRANSACTION RCMTHI;
                     end TRY
                     begin CATCH    
                       if (XACT_STATE()) = -1
                       begin
                         IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION RCMTHI;
                       end
                       else 
                       if (XACT_STATE()) = 1
                       begin
                         IF @@TRANCOUNT > 0 COMMIT TRANSACTION RCMTHI;
                       end  
                       IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION RCMTHI;
	          ...
	          END
                  ----------------------------------------------
                  Insert into #TEMPRecreateMonthlyInvoiceHoldingTable(SQLErrorcode,InvoiceID)
                  select @SQLErrorCode as SQLErrorcode,@LVC_InvoiceID as InvoiceID 
                  ----------------------------------------------
      ...
      IF ( @LI_MonthlyLineItemCount <> 0 ) 
        BEGIN -- process monthly ACS 
	      ...
	      INNER JOIN 
             ...
             AND OI.OrderIDSeq      = OG.OrderIDSeq -- OG.IDSeq s/b unique; but, migrated data causes dups when orders created > 1
	      ...
	      SELECT @LVC_InvoiceID         as InvoiceIDSeq,
                 ...
                 case when OI.LastBillingPeriodFromDate is null then OI.NewActivationStartDate
                     else Invoices.dbo.fn_SetFirstDayOfFollowingMonth(OI.LastBillingPeriodToDate)
                 ...
                 case when OI.LastBillingPeriodToDate is null   then Invoices.dbo.fn_SetLastDayOfMonth(isnull(OI.LastBillingPeriodFromDate,NewActivationStartDate))
                     else Invoices.dbo.fn_SetLastDayOfFollowingMonth(OI.LastBillingPeriodToDate)
	  ...
	  INNER JOIN 
                      (Select X.OrderIDSeq,X.OrderGroupIDSeq,X.OrderItemIDSeq,X.OrderItemRenewalCount,
                       ...
                       and   X.OrderItemIDSeq  = @LBI_OrderItemID
	 ...
	 AND   S.OrderItemIDSeq      = @LBI_OrderItemID
	 ...
	 AND   OI.IDSeq              = @LBI_OrderItemID
	      ...
	      SELECT @LI_MAX = count(SEQ) FROM #TEMPInvoiceGroupFinal WITH (nolock)
	        ...
	        BEGIN --> BEGIN for while loop - Invoice group
             ...
             -- Set the to date for determining the number of monthly invoices to create to the lesser 
             -- of activation end date or target date.
             ...
             SELECT  @LDT_BillingPeriodFromDate = Invoices.dbo.fn_SetFirstDayOfMonth(@LDT_BillingPeriodToDate)
             ...
             WHILE  @MonthlyInvoiceMinCount <= @MontlyInvoiceMaxCount
                BEGIN --: monthly billing while loop begin
                  SELECT @newBillingPeriodFromDate = Invoices.dbo.fn_SetFirstDayOfFollowingMonth(@LDT_BillingPeriodFromDate) --Feb 01 2007 12:00AM
                  SELECT @newBillingPeriodToDate   = Invoices.dbo.fn_SetLastDayOfFollowingMonth(@LDT_BillingPeriodFromDate) --Feb 28 2007 12:00AM 
       ...
       END -- Processing completed for ACS
        ...
        select @imin = 1,@imax=count(*) from  #TEMPRecreateMonthlyInvoiceHoldingTable with (nolock)
        while @imin <= @imax
        begin
          select @LVC_INVOICE = InvoiceID 
          from   #TEMPRecreateMonthlyInvoiceHoldingTable with (nolock)
          where  IDSeq = @imin
        ...
        select distinct SQLErrorcode,InvoiceID from #TEMPRecreateMonthlyInvoiceHoldingTable with (nolock)
        ----SELECT @SQLErrorcode as SQLErrorcode,@LVC_InvoiceID as InvoiceID
        -------------------------------------------------------------------------------------------------
        ---Final Cleanup
        -------------------------------------------------------------------------------------------------
        ...
        DROP TABLE #TEMPRecreateMonthlyInvoiceHoldingTable
        -------------------------------------------------------------------------------------------------
