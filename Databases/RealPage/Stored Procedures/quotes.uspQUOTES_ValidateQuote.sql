...
SET ANSI_NULLS ON
GO
--Exec Quotes.dbo.uspQUOTES_ValidateQuote 'Q1105000338','ApproveQuote', '11/02/2010'

CREATE PROCEDURE [quotes].[uspQUOTES_ValidateQuote] (@IPVC_QuoteID             varchar(50),                                                                                   
                                                  @IPVC_ValidationType      varchar(100)= 'SubmitQuote',
                                                  @IPVC_ValidationDate      varchar(50) = NULL ---> For Quote Submit, pass in Submit Date that the user keys in From UI.
                                                                                               ---> For Quote Approval, pass in Quote Approval Date that the User Keys in From UI.
                                                  )
AS
BEGIN
  ...
  --------------------------------------------------------------------------------------------
  --8.1 :Check if any bundle with autofulfillilfflag and/or autofulfillacsancflag has invalid AutoFulfillStartDate
  --------------------------------------------------------------------------------------------
  if (@IPVC_ValidationType = 'ApproveQuote')
  begin
    insert into #LTBL_Errors(ErrorMsg,[Name],CanOverrideFlag)
    select Grp.[Name] + ' is configured for Order ' + (Case when (grp.autofulfillilfflag = 1 and grp.autofulfillacsancflag = 1) then  'ILF and ACS'
                                                            when (grp.autofulfillilfflag = 1)    then 'ILF'
                                                            when (grp.autofulfillacsancflag = 1) then 'ACS'
                                                      end) +
    ' Order fulfillment with Invalid Auto Fulfill Start Date.' as ErrorMsg,'Input Correct Auto Fulfill Start Date in Quote Approval Modal.' as Name, 0 as CanOverrideFlag
    from  QUOTES.DBO.[Group] grp  with (nolock)
    where grp.QuoteIDSeq = @IPVC_QuoteID
    and   (grp.autofulfillilfflag = 1 or grp.autofulfillacsancflag = 1)
    and   (isdate(grp.AutoFulfillStartDate) = 0 or grp.AutoFulfillStartDate < @LDT_ApprovalDate)
    and   (@IPVC_ValidationType = 'ApproveQuote')
  end
  ...
  and    QI.Chargetypecode   = 'ACS'
  and    (QI.MeasureCode = 'TRAN' or (P.Autofulfillflag = 1 and G.AutoFulfillACSANCFlag = 1)) 
  ...
  begin
    ...
    select (case when O.PropertyIDSeq is NULL then 'Company: ' + Max(Com.Name)
          ...
          end)  + ' has Active Overlapping Agreement(s) for same Product(s): ' +                        
         ...
         (Case 
              when Max(OI.StatusCode) = 'EXPD'
                then 'Currently Expired but was Active from : ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.EndDate,101)+ '.' +char(13)+
                     '[Suggestion(s):--->] (1) If Back bill then Consider Approving ' + @IPVC_QuoteID + ' with Approval/Fulfillment Date that does not overlap with existing active product order(s). ' + char(13)+
               ...
               when OI.Canceldate is not null 
                 then 'Cancelled and Active from : ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.Canceldate,101)+ '.' +char(13)+
                       '[Suggestion(s):--->] (1) Backdate ' + convert(varchar(50),O.OrderIDSeq) + ' with Canceldate :' + convert(varchar(50),coalesce(OI.StartDate,coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate)),101) + char(13)+
                       '                     (2) Consider Approving ' + @IPVC_QuoteID + ' with Approval/Fulfillment Date   :' + convert(varchar(50),OI.Canceldate,101)+ char(13)+
                ...
                when OI.Canceldate is null 
                 then 'Fulfilled and Active from ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.EndDate,101)+ '.' +char(13)+
                      '[Suggestion(s):--->] (1) Cancel ' + convert(varchar(50),O.OrderIDSeq) + ' with Canceldate :  ' + convert(varchar(50),coalesce(OI.StartDate,coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate)),101) + char(13)+
                      '                     (2) Consider Approving ' + @IPVC_QuoteID + ' with Approval/Fulfillment Date : ' + convert(varchar(50),OI.Enddate+1,101)+ char(13)+
                      '                     (3) Else Approve Quote ' + @IPVC_QuoteID + ' with Delay ILF and Delay ACS/ANC to allow quote approval and then fulfil later with Non-overlapping Startdate. ' + char(13)+
                      '                     (4) Do not approve the Quote ' + @IPVC_QuoteID + '. Explore other options with Client Services Manager.' + char(13)
          end)                                                                           as [Name],
          0                                                                              as CanOverrideFlag
    ...
    inner join
            (select GP.CustomerIDSeq as CompanyIDSeq,GP.PropertyIDSeq,G.GroupType,QI.ProductCode,QI.Chargetypecode,
                    Max(G.AutoFulfillILFFlag) as AutoFulfillILFFlag,
                    Max(G.AutoFulfillACSANCFlag) as AutoFulfillACSANCFlag,Max(G.AutoFulfillStartDate) as AutoFulfillStartDate
             from   QUOTES.dbo.QuoteItem QI with (nolock)
             inner join
                    QUOTES.dbo.[Group] G    with (nolock)
    ...
    and  coalesce(O.PropertyIDSeq,'')   = coalesce(S.PropertyIDSeq,'')
    ...
    and  ( (OI.MeasureCode          = 'TRAN') 
               OR 
           (P.Autofulfillflag = 1 and S.AutoFulfillACSANCFlag = 1)
                OR
           (S.AutoFulfillACSANCFlag = 1)               
         )
    -----------------------------------
    and (
         ( coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate) >= OI.Startdate
            and
           coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate) <= coalesce(OI.Canceldate-1,OI.Enddate)
            and
           (OI.Startdate<>OI.Canceldate OR OI.Canceldate is null)
          )
            OR
         ( dateadd(YY,1,coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate)) >= OI.Startdate
            and
           dateadd(YY,1,coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate)) <= coalesce(OI.Canceldate-1,OI.Enddate)
            and
           (OI.Startdate<>OI.Canceldate OR OI.Canceldate is null)
          )
         )
    ...
    group by O.CompanyIDSeq,O.PropertyIDSeq,O.OrderIDSeq,OI.StartDate,OI.Canceldate,OI.EndDate,coalesce(S.AutoFulfillStartDate,@LDT_ApprovalDate)
    ---------------------------------------------------------------------------------------------------------------------
    ...
    begin
      create table #TempOrginalProperty(seq           int identity(1,1) not null primary key,
                                        ...
                                        AutoFulfillACSANCFlag int,AutoFulfillStartDate datetime
                                        );

      insert into #TempOrginalProperty(companyidseq,propertyidseq,propertyname,city,state,zip,AutoFulfillILFFlag,AutoFulfillACSANCFlag,AutoFulfillStartDate)
      select A.companyidseq,A.propertyidseq,Max(Prp.Name) as propertyname,A.city,A.state,A.zip,
             S.AutoFulfillILFFlag,S.AutoFulfillACSANCFlag,S.AutoFulfillStartDate
      ...
      inner join
             (Select GP.propertyidseq,ltrim(rtrim(X.AddressLine1)) as AddressLine1,ltrim(rtrim(X.city)) as city,ltrim(rtrim(X.state)) as state,ltrim(rtrim(X.zip)) as zip,coalesce(Y.Phase,'-1') as Phase,
              ...
              inner join
                    QUOTES.dbo.[Group] G    with (nolock)
              on     G.IDSeq            = GP.GroupIDSeq 
              and    G.QuoteIdSeq       = GP.QuoteIdSeq
              and    G.Quoteidseq       = @IPVC_QuoteID
              and    GP.Quoteidseq      = @IPVC_QuoteID
              and    G.CustomerIDSeq    = @LVC_CompanyID
              and    GP.CustomerIDSeq   = @LVC_CompanyID
              and    (G.AutoFulfillILFFlag = 1 or G.AutoFulfillACSANCFlag = 1)
              inner join
      ...
      insert into #LTBL_Errors(ErrorMsg,Name,CanOverrideFlag)
      select 'Property: ' + Max(Prpty.propertyname)   + ' with a different Account but same address as current property has Active Overlapping Agreement(s) for:' +char(13)+
             Max(QPC.ProductName) + '.(Order #' + convert(varchar(50),O.OrderIDSeq) + ').' + char(13) as ErrorMsg, 
            (Case 
                 when Max(OI.StatusCode) = 'EXPD'
                   then 'Currently Expired but was Active from : ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.EndDate,101)+ '.' +char(13)+
                 ...
                 when OI.Canceldate is not null 
                   then 'Cancelled and Active from : ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.Canceldate,101)+ '.' +char(13)+
                        '[Suggestion(s):--->] (1) Backdate ' + convert(varchar(50),O.OrderIDSeq)  + ' with Canceldate :' + convert(varchar(50),coalesce(OI.StartDate,coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate)),101) + char(13)+
                        '                     (2) Consider Approving ' + @IPVC_QuoteID       + ' with Approval/Fulfillment Date  :' + convert(varchar(50),OI.Canceldate,101)+ char(13)+
                        '                     (3) Else Approve Quote ' + @IPVC_QuoteID + ' with Delay ILF and Delay ACS/ANC to allow quote approval and then fulfil later with Non-overlapping Startdate. ' + char(13)
                 when OI.Canceldate is null 
                   then 'Fulfilled and Active from ' + convert(varchar(50),OI.StartDate,101) + '-' + convert(varchar(50),OI.EndDate,101)+ '.' +char(13)+
                        '[Suggestion(s):--->] (1) Cancel ' + convert(varchar(50),O.OrderIDSeq)  + ' with Canceldate : ' + convert(varchar(50),coalesce(OI.StartDate,coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate)),101) + char(13)+
                        '                     (2) Else Consider Approving ' + @IPVC_QuoteID     + ' with Approval/Fulfillment Date : ' + convert(varchar(50),OI.Enddate+1,101)+ char(13)+
                        '                     (3) Else Approve Quote ' + @IPVC_QuoteID + ' with Delay ILF and Delay ACS/ANC to allow quote approval and then fulfil later with Non-overlapping Startdate. ' + char(13)
            end)                                                                           as [Name],
            0                                                                              as CanOverrideFlag
      from   ORDERS.dbo.[ORDER] O With (nolock)
     ...
     and  ( (OI.MeasureCode          = 'TRAN') 
               OR 
           (P.Autofulfillflag = 1 and Prpty.AutoFulfillACSANCFlag = 1)
                OR
           (Prpty.AutoFulfillACSANCFlag = 1)               
         )
     -----------------------------------
     and (
         ( coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate) >= OI.Startdate
            and
           coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate) <= coalesce(OI.Canceldate-1,OI.Enddate)
            and
           (OI.Startdate<>OI.Canceldate OR OI.Canceldate is null)
          )
            OR
         ( dateadd(YY,1,coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate)) >= OI.Startdate
            and
           dateadd(YY,1,coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate)) <= coalesce(OI.Canceldate-1,OI.Enddate)
            and
           (OI.Startdate<>OI.Canceldate OR OI.Canceldate is null)
          )
         )
     -----------------------------------
      group by O.CompanyIDSeq,O.PropertyIDSeq,O.OrderIDSeq,OI.StartDate,OI.Canceldate,OI.EndDate,coalesce(Prpty.AutoFulfillStartDate,@LDT_ApprovalDate)
    end
  end 
  --------------------------------------------------------------------------------------------
  if (@IPVC_ValidationType = 'ApproveQuote') and exists (select Top 1 1 from #LTBL_Errors with (nolock))
