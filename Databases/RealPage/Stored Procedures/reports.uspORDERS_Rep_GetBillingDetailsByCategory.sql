...
-- Procedure Name  : uspORDERS_Rep_GetBillingDetailsByCategory      
...
--            
-- Code Example    : Exec [dbo].[uspORDERS_Rep_GetBillingDetailsByCategory] '','','','','','','05/05/2009','05/30/2009'
--       
-- Revision History:      
...
------------------------------------------------------------------------------------------------------      
CREATE PROCEDURE [reports].[uspORDERS_Rep_GetBillingDetailsByCategory]      
                                                                  (      
...
BEGIN         
  ...
  ------------------------------------------------------------
  create table #LT_BillingDetailsByCategory(sortseq                int not null identity(1,1),
  ...
  ------------------------------------------------------------
  Insert into #LT_BillingDetailsByCategory(companyidseq,propertyidseq,companyname,propertyname,invoiceidseq,printflag,
  ...
  select I.companyIDSeq,I.PropertyIDSeq,I.CompanyName,I.propertyname,I.InvoiceIDSeq,I.PrintFlag,
         ...
         Max(coalesce(II.Units,I.Units,OI.Units))  as ACSUnits, 
         Max(coalesce(II.Units,I.Units,OI.Units))  as ACSBeds,  
         Max(coalesce(II.PPUPercentage,I.PPUPercentage,OI.PPUPercentage))  as ACSPPUPercentage,
  ...
  ------------------------------------------------------------------------------
  Update #LT_BillingDetailsByCategory set RecordIdentifier = (case when custombundleflag = 1 then 'CPR'
  ...
  from   #LT_BillingDetailsByCategory D with (nolock)
  inner join
  ...
  ------------------------------------------------------------------------------
  set identity_insert #LT_BillingDetailsByCategory on;
  ---Roll Up for Custom Bundle
  Insert into #LT_BillingDetailsByCategory(sortseq,companyidseq,propertyidseq,companyname,propertyname,invoiceidseq,printflag,orderidseq,ordergroupidseq,ordergroupname,
  ...
  from   #LT_BillingDetailsByCategory S with (nolock)
  ...
  group  by S.companyidseq,S.propertyidseq,S.companyname,S.propertyname,S.invoiceidseq,S.printflag,S.orderidseq,S.ordergroupidseq,S.ordergroupname,
            S.BillingPeriodFromDate,S.BillingPeriodToDate; 
  set identity_insert #LT_BillingDetailsByCategory off;  
 
  Update #LT_BillingDetailsByCategory  set    
  ...
  set identity_insert #LT_BillingDetailsByCategory off;   
   ...
   select 
          ...
          coalesce(categoryname,'')                                                                       as categoryname,        
          (case when s.recordidentifier not in ('CPR','TPR') then coalesce(companyname,'') else '' end)   as companyname,
          (case when s.recordidentifier not in ('CPR','TPR') then coalesce(propertyname,'') else '' end)  as propertyname,
          (case when s.recordidentifier not in ('CPR','TPR') then coalesce(accountname,'') else '' end)   as accountname,           
          (case when s.recordidentifier not in ('CPR','TPR') then coalesce(orderidseq,'') else '' end)    as orderidseq,
          coalesce(productname,'')                                                                        as productname,
          ---------------------------------------------
          coalesce(ilforderitemstatus,'')                                                            as ilforderitemstatus,
          Convert(datetime,ilfstartdate)							                                 as ilfstartdate,
          Convert(datetime,ilfcanceldate)							                                 as ilfcanceldate,
          convert(numeric(10,2),ilfextchargeamount)													 as ilflistamount,
          convert(numeric(10,2),ilfdiscamount)														 as ilfdiscamount,
          convert(numeric(10,2),ilfdiscpercent)														 as ilfdiscpercent,
          convert(numeric(10,2),ilfnetchargeamount)													 as ilfnetchargeamount,
          convert(numeric(10,2),ilfbilledamount)													 as ilfbilledamount,
          convert(numeric(10,2),ilfcreditamount)													 as ilfcreditamount,
          convert(numeric(10,2),ilfpendingamount)													 as ilfpendingamount,
          ---------------------------------------------
          ...
          (case when acsenddate is not null then (acsstartdate + ' - ' + acsenddate)
																				                     ...
																				                     as acsperiod,
          Convert(datetime,acscanceldate)						                                     as acscanceldate,
          ...
          convert(numeric(10,2),acsunitprice)														 as acsunitprice,
          convert(numeric(10,2),acsextchargeamount)													 as acslistamount,
          convert(numeric(10,2),acsdiscamount)														 as acsdiscamount,
          convert(numeric(10,2),acsdiscpercent)													     as acsdiscpercent,
          convert(numeric(10,2),acsnetchargeamount)													 as acsnetchargeamount,
          convert(numeric(10,2),acsbilledamount)													 as acsbilledamount,
          convert(numeric(10,2),acscreditamount)													 as acscreditamount,
          convert(numeric(10,2),acspendingamount)													 as acspendingamount,
          ---------------------------------------------
          ...
          (case when ancenddate is not null then (ancstartdate + ' - ' + ancenddate)
                                                                                                  ...
                                                                                                  as ancperiod,
          Convert(datetime,anccanceldate)		                                                  as anccanceldate,
          coalesce(ancmeasure,'')                                                                 as ancmeasure,
          coalesce(ancfrequency,'')                                                               as ancfrequency,
          convert(numeric(10,2),ancunitprice)													  as ancunitprice,
          convert(numeric(10,2),ancextchargeamount)												  as anclistamount,
          convert(numeric(10,2),ancdiscamount)													  as ancdiscamount,
          convert(numeric(10,2),ancdiscpercent)													  as ancdiscpercent,
          convert(numeric(10,2),ancnetchargeamount)												  as ancnetchargeamount,
          convert(numeric(10,2),ancbilledamount)												  as ancbilledamount,
          convert(numeric(10,2),anccreditamount)												  as anccreditamount,
          convert(numeric(10,2),ancpendingamount)												  as ancpendingamount,
          ---------------------------------------------   
  ...
  -------------------------------------------------------------------------- 
  --Final CleanUp
  if (object_id('tempdb.dbo.#LT_BillingDetailsByCategory') is not null) 
  begin
    drop table #LT_BillingDetailsByCategory
  end 
  --------------------------------------------------------------------------          
END 
GO
