...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [documents].[uspDOCUMENTS_GetFootNotesAsXML] 
(
	@InvoiceNotes        XML output, 
...
BEGIN 
  ...
  create table #temp_documenttoprintonInvoice (IDSeq bigint identity(1,1),Description varchar(4000) NULL)
  select @LVC_CompanyIDSeq = I.CompanyIDSeq,@LVC_PropertyIDSeq = I.PropertyIDSeq,@LVC_AccountIDSeq = I.AccountIDSeq
  from   INVOICES.DBO.INVOICE I with (nolock) where  I.InvoiceIDSeq = @IPVC_InvoiceIDSeq
  ...
  Set @InvoiceNotes = ( 
                      ...
                      order by IDSeq asc For XML Path
                    )
  -----------------------------------------------------------
  --Final Cleanup
