...
--                 
------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [reports].[uspINVOICES_Rep_GetInvoiceInfo] (
...
BEGIN 
  ...
  insert into @LTBL_RESULTSET (InvoiceNo, NextInvoiceDate, CurrentILF, CurrentAccess, CurrentTransaction,
                               CurrentSubTotal, CurrentTaxAmount, CurrentCreditAmount)
  select InvoiceIDSeq, '*****', ILFChargeAmount, AccessChargeAmount, TransactionChargeAmount,
         (isnull(ILFChargeAmount,0) + isnull(AccessChargeAmount,0) + isnull(TransactionChargeAmount,0)
          + isnull(ShippingandHandlingAmount,0)
         ), TaxAmount, CreditAmount
  from Invoices.dbo.Invoice with (nolock)
  where InvoiceIDSeq = @IPVC_InvoiceID

  select @LVC_AccountID = AccountIDSeq
  from Invoices.dbo.Invoice with (nolock)
  where InvoiceIDSeq = @IPVC_InvoiceID

  update T
  set T.CurrentTotalCharges = isnull(T.CurrentSubTotal,0) + isnull(T.CurrentTaxAmount,0),
  ...
  Select @LN_LastInvoice = max(InvoiceIDSeq)
  From Invoices.dbo.Invoice with (nolock)
...
if (@LN_LastInvoice<>'')
  ...
  */
  ...
  update @LTBL_RESULTSET
  set PastDue = @LN_TotalInvoiceAmtExcludingCurrent - @LN_TotalInvoiceAmtPaid
  

  update @LTBL_RESULTSET
  set TotalOutstandingCharges = PastDue + CurrentGrossCharges

  update @LTBL_RESULTSET
  set LastInvoiceDate    = @LN_LastInvoiceDate,
      LastInvoiceCharges = @LN_LastInvoiceamt,
      LastInvoiceDueDate = @LN_LastInvoiceDueDate,
      LastPaymentDate = '',
...
--                                  ((PastDue + CurrentTotalCharges) - (LastPaymentAmount+ CurrentCreditAmount))
--                                  ,1,2)
--                 )                    as EndingBalance
    '$' + Convert(varchar(15), Invoices.dbo.fn_FormatCurrency(
                                  (CurrentTotalCharges),1,2)
                 )                    as EndingBalance
  from @LTBL_RESULTSET

--  update Invoice
--  set PrintFlag = 1,
--  OriginalPrintDate = case when OriginalPrintDate is null then getdate() else OriginalPrintDate end,
--  ReprintDate = getdate(),
