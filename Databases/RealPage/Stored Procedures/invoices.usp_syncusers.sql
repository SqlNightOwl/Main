...
SET ANSI_NULLS ON
GO
create proc [invoices].[usp_syncusers]    
AS
begin
  ...
  union
  select 'grant exec on [' + name + '] to OMSReadOnly; '  from sysobjects where type in ('P','FN', 'IF','FS', 'FT')
         and name in ('uspINVOICES_DeleteTWT_InvoicesWithNoPDF','uspINVOICES_InsertTWT_InvoicesWithNoPDF')
  union
