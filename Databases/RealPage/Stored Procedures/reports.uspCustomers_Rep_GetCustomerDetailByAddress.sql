...
-- 06/21/2011      : Naval Kishore Modified to get Commercial Flag, Tfs ID # 566
-- 08/16/2011      : Mahaboob Modified to introduce CustomerType in Search Critieria. Tfs ID # 989
------------------------------------------------------------------------------------------------------      
CREATE PROCEDURE [reports].[uspCustomers_Rep_GetCustomerDetailByAddress]      
(      
 ...
 @IPVC_CustomerType VARCHAR(8000)='',
 @IPI_GSAEntityFlag INT
 )      
AS      
BEGIN           
SET NOCOUNT ON     
...
DECLARE @LT_CustomerType TABLE (CustomerTypeCode CHAR(50))  
DeCLARE @LVC_CustomerTypeCode VARCHAR(8000)  
SELECT  @LVC_CustomerTypeCode = @IPVC_CustomerType
DECLARE @LVC_Delimiter char(1), @SQLErrorCode int, @SQLRowCount int  
SET @LVC_Delimiter = ','  
  
IF (@LVC_CustomerTypeCode IS NULL OR LTRIM(RTRIM(@LVC_CustomerTypeCode)) = '') SET @LVC_CustomerTypeCode = ''  
IF LEN(@LVC_CustomerTypeCode) > 0 AND CHARINDEX(@LVC_CustomerTypeCode,'',1) > 0 SET @LVC_CustomerTypeCode = '' -- If all is one selection, set to all  
IF @LVC_CustomerTypeCode <> ''   
BEGIN  
 --Parse the string to get all the parameters passed  
 INSERT INTO @LT_CustomerType ([CustomerTypeCode])  
 SELECT [Items] as [CustomerTypeCode] from OMSREPORTS.dbo.fnSplitDelimitedString(@LVC_CustomerTypeCode,@LVC_Delimiter)  
 ...
 begin  
  PRINT 'A SQL Error occurred, ' + CAST(@SQLErrorCode AS VARCHAR(10)) + ', attempting to get '  
   + 'the Customer Type Code parameters passed.'  
  RETURN -1  
 end  
END  

DECLARE @MultiFamilyFlag int, @VendorFlag int, @IncludeGovtEntityFilter int
SELECT  @MultiFamilyFlag = 0, @VendorFlag = 0, @IncludeGovtEntityFilter = 1
SELECT  @MultiFamilyFlag = 1  FROM @LT_CustomerType WHERE CustomerTypeCode = 'MultiFamilyFlag'
SELECT  @VendorFlag      = 1  FROM @LT_CustomerType WHERE CustomerTypeCode = 'VendorFlag'



IF ( @IPI_GSAEntityFlag = 2 )
BEGIN
     SELECT @IncludeGovtEntityFilter = 0
END
 ...
 SELECT C.IDSeq                                                             AS CustomerID,  
    ...
    END)            AS SeparateInvoiceByFamilyFlag,  
	  ...
	  (Case when C.StatusTypeCode ='ACTIV' Then 'Active' else 'Inactive' end) as Status,				  
	   ...
	   (CASE WHEN MAX(convert(int,C.MultiFamilyFlag)) = 1 THEN 'Yes'  
		ELSE 'No'                                       
		END)																			  AS [Multi Family],
		(CASE WHEN MAX(convert(int,C.GSAEntityFlag)) = 1 THEN 'Yes'  
		ELSE 'No'                                       
		END)																			  AS [Govt Entity],
		(CASE WHEN MAX(convert(int,C.VendorFlag)) = 1 THEN 'Yes'  
		ELSE 'No'                                       
		END)																			  AS [Vendor] 
...
AND (((C.MultiFamilyFlag = @MultiFamilyFlag   and @MultiFamilyFlag         = 1 AND ((C.GSAEntityFlag   = @IPI_GSAEntityFlag and @IncludeGovtEntityFilter = 1) or (@IncludeGovtEntityFilter = 0 and C.GSAEntityFlag   = C.GSAEntityFlag  ))))
   OR ((C.VendorFlag      = @VendorFlag       and @VendorFlag              = 1 AND ((C.GSAEntityFlag   = @IPI_GSAEntityFlag and @IncludeGovtEntityFilter = 1) or (@IncludeGovtEntityFilter = 0 and C.GSAEntityFlag   = C.GSAEntityFlag  )))))
...
Select C.IDSeq                                                                            AS CustomerID,  
    ...
    END)            AS SeparateInvoiceByFamilyFlag,  
	 ...
	 --  (CASE WHEN MAX(convert(int,C.MultiFamilyFlag)) = 1 THEN 'Yes'  
		...
		--END)	
		'N/A'																			  AS [Multi Family],
		(CASE WHEN MAX(convert(int,P.GSAEntityFlag)) = 1 THEN 'Yes'  
		ELSE 'No'                                       
		END)																			  AS [Govt Entity],
		(CASE WHEN MAX(convert(int,P.VendorFlag)) = 1 THEN 'Yes'  
		ELSE 'No'                                       
		END)																			  AS [Vendor]    
