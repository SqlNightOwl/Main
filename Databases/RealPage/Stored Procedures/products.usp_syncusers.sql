...
SET ANSI_NULLS ON
GO
create proc [products].[usp_syncusers]    
AS
begin
  ...
  if user_id('SharePointReader') is null 
  begin
    EXEC sp_adduser 'SharePointReader', 'SharePointReader'
  end  
  EXEC sp_change_users_login 'Auto_Fix', 'SharePointReader'; 
  -----------------------------------------------
  ...
  union
  select 'grant select on [' + name + '] to LSDTransactionUser; '  
  from sysobjects where type in ('V','U','TF') and 
  name in ('Category','ReportingType','ChargeType','Family','Frequency',
           'Measure','Platform','ScreeningProductMapping','Charge',
           'Product','ProductType','Market')
  union
  select 'grant exec on [' + name + '] to SharePointReader; '  from sysobjects where type in ('P','FN', 'IF','FS', 'FT')
  and name in ('uspPRODUCTS_PlatformList','uspPRODUCTS_FamilyList','uspPRODUCTS_GetCategory','uspPRODUCTS_GetProductType',
               'uspPRODUCTS_ReportingTypeList','uspProducts_GetRevenueRecognitionList',
               'uspPRODUCTS_GetFrequency','uspPRODUCTS_GetMeasure',
               'uspPRODUCTS_GetChargeTypeList','uspPRODUCTS_GetReportingType',
               'uspPRODUCTS_GetProductAttributes','uspPRODUCTS_GetChargeAttributes',
               'uspPRODUCTS_GetCategoryByInput','uspPRODUCTS_GetProductTypeByInput'                  
              ) 
  set @LI_MIN = 1
