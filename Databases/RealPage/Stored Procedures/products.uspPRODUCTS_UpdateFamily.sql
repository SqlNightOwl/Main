...
-- Procedure Name  : uspPRODUCTS_UpdateFamily
-- Description     : This proc is called by Product Administration Family Management to Update Existing Familys
...
-- Code Example    : 
/*
Exec PRODUCTS.dbo.uspPRODUCTS_UpdateFamily 
                      @IPVC_FamilyCode = 'NFM'                           --> This is the existing Familycode 
                      ,@IPVC_FamilyName = 'New Family'                   --> This is the existing FamilyName 
...
------------------------------------------------------------------------------------------------------
Create PROCEDURE [products].[uspPRODUCTS_UpdateFamily]  (@IPVC_FamilyCode                 varchar(3),               ---->MANDATORY : This is the upto 3 character Unique Family Code for adding existing Family
                                                    @IPVC_FamilyName                 varchar(70),              ---->MANDATORY : This is Family Name for the existing Family 
...
BEGIN --> Main Begin
  ...
  ------------------------------------------
  --Validation 2: @IPVC_FamilyName should be unique and should not already exists
  if exists (select  Top 1 1
             from    PRODUCTS.dbo.Family PT with (nolock)
             where   (PT.Name =  @IPVC_FamilyName)
             and     (PT.Code <> @IPVC_FamilyCode)   
            )
  begin
    select @LVC_CodeSection = 'Error: Family Name already exists for a different Family Code in the system. Family Name should be unique.';
    Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError  @IPVC_CodeSection = @LVC_CodeSection;
    return;
  end
  ------------------------------------------
  if exists (select  Top 1 1
             ...
             and     (
                      (FM.Name                  <> @IPVC_FamilyName)
                         OR
                      (FM.Description           <> coalesce(@IPVC_FamilyDescription,@IPVC_FamilyName))
                         OR
                      (FM.EpicorPostingCode     <> @IPVC_FamilyEpicorPostingCode)
                         OR
                      (FM.TaxwareCompanyCode    <> @IPVC_FamilyTaxwareCompanyCode)
                         OR
                      (FM.BusinessUnitLogo      <> @IPVC_FamilyBusinessUnitLogo)
                         OR
                      (FM.PrintFamilyNoticeFlag <> @IPI_PrintFamilyNoticeFlag)
                     )
            )
  begin
    Update FM
    set    FM.Name                  = @IPVC_FamilyName
          ,FM.Description           = coalesce(@IPVC_FamilyDescription,@IPVC_FamilyName)
          ,FM.EpicorPostingCode     = @IPVC_FamilyEpicorPostingCode
          ,FM.TaxwareCompanyCode    = @IPVC_FamilyTaxwareCompanyCode
          ,FM.BusinessUnitLogo      = @IPVC_FamilyBusinessUnitLogo
          ,FM.PrintFamilyNoticeFlag = (Case when (convert(int,FM.PrintFamilyNoticeFlag) <> @IPI_PrintFamilyNoticeFlag)
                                                then @IPI_PrintFamilyNoticeFlag
                                               else FM.PrintFamilyNoticeFlag
                                          end)
          ,FM.ModifiedByIdSeq = @IPBI_UserIDSeq
          ,FM.ModifiedDate    = @LDT_SystemDate 
          ,FM.SystemLogDate   = @LDT_SystemDate
    from  PRODUCTS.dbo.Family FM with (nolock)
    where   (FM.Code =  @IPVC_FamilyCode) 
    and     (
              (FM.Name               <> @IPVC_FamilyName)
