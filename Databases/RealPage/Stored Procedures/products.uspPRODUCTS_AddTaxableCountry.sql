...
-- Procedure Name  : uspPRODUCTS_AddTaxableCountry
-- Description     : This proc is called by Product Administration TaxableCountry Management to Add New TaxableCountry(s)
...
-- Code Example    : 
/*
Exec PRODUCTS.dbo.uspPRODUCTS_AddTaxableCountry 
                       @IPVC_TaxableCountryCode = '12'                --> This is the new TaxwareCompanyCode 
                      ,@IPVC_TaxableCountryCode = 'USA'               --> This is the new TaxableCountrycode 
...
------------------------------------------------------------------------------------------------------
Create PROCEDURE [products].[uspPRODUCTS_AddTaxableCountry]  (@IPVC_TaxwareCompanyCode          varchar(20),    ---> MANDATORY : This is TaxwareCompanyCode. This is upto 10 characters.
                                                         @IPVC_TaxableCountryCode          varchar(20),    ---> MANDATORY : This is the upto 3 character Unique TaxableCountry Code for adding new TaxableCountry
...
BEGIN --> Main Begin
  ...
  begin
    select @LVC_CodeSection = 'Error: Valid TaxableCountry Code should be 3 characters. 3-Letter Standard UN CountryCode(s) are accepted. Refer: http://countrycode.org/ OR http://www.worldatlas.com/aatlas/ctycodes.htm';
    Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError  @IPVC_CodeSection = @LVC_CodeSection;
    return;
  end
  ...
  begin
    select @LVC_CodeSection = 'Error: Country Code is not found or set up yet. Please Set up a valid Country with 3-Letter Standard UN CountryCode(s) in Customers.dbo.Country. Refer: http://countrycode.org/ OR http://www.worldatlas.com/aatlas/ctycodes.htm';
    Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError  @IPVC_CodeSection = @LVC_CodeSection;
    return;
  end 
  ------------------------------------------
  --Validation 2: @IPVC_TaxwareCompanyCode and @IPVC_TaxableCountryCode should be unique and should not already exists
  ...
  begin
    select @LVC_CodeSection = 'Error: This TaxableCountryCode and TaxwareCompanyCode combination already exists in the system. TaxableCountryCode and TaxwareCompanyCode combination should be unique for new TaxableCountry additions.';
    Exec CUSTOMERS.DBO.uspCUSTOMERS_RaiseError  @IPVC_CodeSection = @LVC_CodeSection;
    return;
  end  
  ------------------------------------------
  ...
  begin
    Insert into Products.dbo.TaxableCountry(TaxwareCompanyCode,TaxableCountryCode,CalculateTaxFlag,CreatedByIDSeq,CreatedDate,SystemLogDate)
    select @IPVC_TaxwareCompanyCode as TaxwareCompanyCode,@IPVC_TaxableCountryCode as TaxableCountryCode,
           @IPI_CalculateTaxFlag as CalculateTaxFlag,
