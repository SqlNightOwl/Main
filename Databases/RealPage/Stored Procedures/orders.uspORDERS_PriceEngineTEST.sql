...
SET ANSI_NULLS ON
GO
--exec ORDERS.DBO.uspORDERS_PriceEngineTEST @IPVC_OrderID='O0712078844',@IPI_GroupID=42405
CREATE PROCEDURE [orders].[uspORDERS_PriceEngineTEST] (@IPVC_OrderID        varchar(50),
...
BEGIN   
  ...
  insert into #TEMP_PropertiesProductsHoldingTable
             (orderid,groupid,orderitemid,statuscode,renewaltypecode,
              ...
              grouptype,productcode,productdisplayname,
  ...
  Set    T.extchargeamount = (case 
          ...
          PricingLineItemNotes = PricingLineItemNotes + (case when (T.grouptype <> 'PMC')           and             
                                        (T.sites = 0 and T.units = 0)
                                      then ''                               
                                   ---------------------------------------
                                   when (T.propertythresholdoverride=1) 
  ...
  set  D.weightofProducttototal = ((convert(float,D.extchargeamount))/
                                   (case when D.Chargetypecode = 'ILF' 
                                            then (case when S.ILFBundleTotalextchargeamount=0 then 1 else S.ILFBundleTotalextchargeamount end)
                                         when D.Chargetypecode = 'ACS' 
                                            then (case when S.ACSBundleTotalextchargeamount=0 then 1 else S.ACSBundleTotalextchargeamount end)
                                         else (case when D.extchargeamount=0 then 1 else D.extchargeamount end)
                                     end)
  ...
  -----------------------------------------------------------------------------------------
----???
select * from #TEMP_PropertiesProductsHoldingTable D with (nolock)
  inner join
       (Select  X.groupid   as    groupid,
                X.orderid   as    orderid,
                X.custombundlenameenabledflag as custombundlenameenabledflag,                
  ...
  */
  Update T
    set T.unitofmeasure          = (Case when (T.measurecode = 'SITE') and (T.Quantityenabledflag = 0)
        ...
        T.extyear1chargeamount   =  (T.extchargeamount * (case when (T.quantityenabledflag=1 and T.quantitymultiplierflag=0) then 1
  ...
  select T.orderid                                           as orderid,
         ...
         (case 
              when (T.chargeamount) >= 0 
                 then ((T.chargeamount)- (T.netchargeamount))*(100)/
                      (case when (T.chargeamount)=0 then 1
                            else (T.chargeamount)
                      end)
               when (T.extchargeamount) > 0 
         ...
         (case when (T.extchargeamount) > 0 
           ...
           -------------------------------------------------------------------
          (T.extchargeamount)                               as extchargeamount,
          (T.extSOCchargeamount)                            as extSOCchargeamount,
         ------------------------------------------------------------------------ 
         ...
         (T.extyear1chargeamount)                         as extyear1chargeamount,         
         (T.netchargeamount)                              as netchargeamount,
         (T.netextchargeamount)                           as netextchargeamount,
         (T.netextyear1chargeamount)                      as netextyear1chargeamount,         
