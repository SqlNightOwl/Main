...
/*
--------------------------------------------------------------------------------
----Discount in NUMBER :  @IPVC_ChargeType = 'ILF' or 'Y1' or 'Y2' or 'Y2'
--------------------------------------------------------------------------------
Exec uspQUOTES_GraphBPDiscounts @IPC_CompanyID = 'A0000002216',@IPVC_QuoteID = 32,
     @IPI_ScaleFactor=1,@IPVC_ChargeType='ILF',
     @IPVC_DisplayNumOrPercent='NUMBER',@IPVC_RETURNTYPE = 'RECORDSET'
--------------------------------------------------------------------------------
----Discount in PERCENT : @IPVC_ChargeType = 'ILF' or 'Y1' or 'Y2' or 'Y2'
--------------------------------------------------------------------------------
Exec uspQUOTES_GraphBPDiscounts @IPC_CompanyID = 'A0000002216',@IPVC_QuoteID = 32,
...
----------------------------------------------------------------------------
*/
CREATE PROCEDURE [quotes].[uspQUOTES_GraphBPDiscounts] (@IPC_CompanyID             varchar(11),
  ...
  begin --> Begining of @LT_BPQuoteProductdetails populate    
    ...
    begin
      ...
      select seq,quoteid,productcode,productname,productdisplayname,
             sum(ilfdiscountamount)*100/(case when @LN_totalilfdiscountamount=0 then 1 else @LN_totalilfdiscountamount end),
             sum(accessyear1discountamount)*100/(case when @LN_totalaccessyear1discountamount=0 then 1 else @LN_totalaccessyear1discountamount end),
             sum(accessyear2discountamount)*100/(case when @LN_totalaccessyear2discountamount=0 then 1 else @LN_totalaccessyear2discountamount end),
             sum(accessyear3discountamount)*100/(case when @LN_totalaccessyear3discountamount=0 then 1 else @LN_totalaccessyear3discountamount end)
    ...
    begin
      ...
      select seq,quoteid,productcode,productname,productdisplayname,
             ...
             sum(accessyear2discountamount)/@IPI_ScaleFactor,
             sum(accessyear3discountamount)/@IPI_ScaleFactor
      from @LT_BPQuoteProductdetails where seq <= 3
      group by  seq,quoteid,productcode,productname,productdisplayname
      order by  seq asc
